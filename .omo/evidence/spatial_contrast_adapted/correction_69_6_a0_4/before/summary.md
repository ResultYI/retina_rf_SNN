...
STATUS: **STOP_INIT_CONTRACT_MISMATCH** — 44 single-start fits executed; **not an accepted complete frozen benchmark**.

## Execution deviation and stop

The official runner initializes `a=running_resp.max()` over the entire training response array, before temporal cropping. This run instead used the maximum inside the fitting loss mask. Direct raw-data checking found equality for 21/22 cells; **69#6** has full-training maximum **4**, fitting-mask maximum **3**, and both executed fits initialized **a0=3**.

The initialization implementation and regression test are now corrected to use the complete training response maximum. **No fit was restarted or rerun.** The only missing execution is the two corrected 69#6 fits, which requires authorization under the no-restart instruction. All numerical results below remain the actually observed, uncorrected 22-cell outputs; they must not be labelled fully compliant final benchmark results. Existing LN and Canonical results are unaffected.

Evidence: `initialization_count_check.json`. Executed source snapshots and their exact run-time SHA256 identity are retained in `executed_source/` and `output_verification.json`. The stored preflight initializer describes what was actually run, not the subsequent correction.
...
The inherited LN artifacts retain their training-only inner-development lambda/step selection and full-training refit. SC performs one full-training output fit with L-BFGS-B, official bounds `a>=0`, others unbounded, official defaults `options=None`, and analytic gradients. The fitting objective is Bernoulli NLL sum divided by fitting event count (official response-sum normalization); reported NLL divides by the number of valid bins. No new regularizer, restart, candidate search, or validation selection was used.
...
## Observed validation NLL (includes the 69#6 initialization deviation)
...
|---|---:|---:|---:|---:|---:|
| overall | 22 | 0.458292199656 | 0.461359484251 | 0.425997944041 | 0.438956145536 |
| MC_ON | 5 | 0.422290021842 | 0.426130383206 | 0.395894259214 | 0.428506755829 |
| MC_OFF | 4 | 0.503537852554 | 0.504707001565 | 0.430339150131 | 0.441669881344 |
...
| 69#6 | MC_OFF | 0.484774227875 | 0.484304416509 | 0.430207490921 | 0.435077965260 | FAIL: a0=3, required 4 |
...
## Verification and numerical warnings
...
- Initializer regression red before correction, green after correction; relevant final suite **8 passed**. Four new non-vendored files passed the scoped rule checker; modified Python files compile/import.
...
- Existing Canonical V1, LN source artifacts, raw data, split and loss source files were unchanged. Only this task's new initialization implementation/test/report changed after execution, with exact executed versions preserved.

## Files

- `results.json`: numerical results plus explicit noncompliance/status metadata.
- `per_cell.csv`, `group_summary.csv`: per-cell and equal-cell aggregate NLLs.
- `preflight.json`: exact executed contract, filter/data checks, git and source hashes.
