...
from models.mechanistic_retina.canonical_contract import validate_canonical_config
...
def validate_v1_checkpoint(checkpoint: Mapping[str, CheckpointValue]) -> None:
    ...
    if checkpoint.get("revision") != MECHANISTIC_MODEL_REVISION or set(model_config) != {
        field.name for field in fields(MechanisticRetinaConfig)
    }:
        raise RFValidationError("Canonical V1 checkpoint revision/config identity mismatch")
    try:
        validate_canonical_config(MechanisticRetinaConfig(**model_config))
    except (TypeError, ValueError) as error:
        raise RFValidationError(f"Canonical V1 config identity mismatch: {error}") from error
    model_state = checkpoint.get("model")
    if not isinstance(model_state, Mapping):
        raise RFValidationError("Canonical V1 checkpoint model state is missing")
    for key, contract in (
        ("_causal_contract_id", CANONICAL_CAUSAL_CONTRACT),
        ("_spatial_contract_id", CANONICAL_SPATIAL_CONTRACT),
    ):
        identity = model_state.get(key)
        expected = torch.tensor(list(contract.encode()), dtype=torch.uint8)
        if not isinstance(identity, torch.Tensor) or identity.dtype != torch.uint8 or not torch.equal(identity.cpu(), expected):
            raise RFValidationError(f"Canonical V1 checkpoint contract identity mismatch: {key}")
