...
from models.mechanistic_retina.support_partition import SupportPartition
...
def validate_pathway_spatial_geometry(
    ...
    *,
    ...
    expected_supports: SupportPartition,
) -> torch.Tensor:
    ...
    if (
        not torch.equal(geometry.bc_support.cpu(), expected_supports.bc.cpu())
        or not torch.equal(geometry.ac_support.cpu(), expected_supports.ac.cpu())
    ):
        raise PathwaySpatialGeometryError(
            "Canonical V1 custom BC/AC support must equal the complete radius-defined disks"
        )
