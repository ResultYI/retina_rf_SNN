...
Use the tools and argument schemas actually available in the current session. The OpenCode and `multi_agent_v1` names below are examples, not required APIs: map them to available equivalents such as `collaboration` when exposed, including the actual history and wait parameters. Do not install tooling or stop the entire task merely because an example tool name is absent.
...
Role-specific behavior must be described in a self-contained `message`. Use the current tool's supported history option, and include needed context, files, diffs, constraints, and skill names. Select an available named role when appropriate; otherwise describe the role in the assignment. If delegation is unavailable or a bounded recovery attempt cannot produce a deliverable, root may perform authorized review work only where it satisfies the required independence: an implementer cannot replace its own independent reviewer. Otherwise record that lane as INCONCLUSIVE. This section governs all tool examples below; unavailable examples do not grant installation or external-action authority.
...
Cover the five review perspectives below, using independent subagents in parallel when available. Each required perspective must have adequate evidence and the required independence for the review to pass. If even one finds a verified defect, the review fails; a missing perspective is INCONCLUSIVE, not a defect. Explicit user or project requirements for separate independent reviewers remain binding.

When `review-work` is a final implementation, PR, or `$start-work` gate,
required evidence remains blocking for approval. FAIL means an evidenced
defect; INCONCLUSIVE means the evidence was not obtained. A timeout,
missing deliverable, ack-only response, or external blocker is not a pass
and does not by itself justify product changes or a debugging loop.
After the bounded recovery below or an equivalent independent review,
report any unresolved lane as INCONCLUSIVE. This permits ending the review
report, not declaring all verification passed, waiving a mandatory gate,
or handing off a PR before the required checks pass. Preserve any explicit
final user-acceptance or action-time approval as a separate pending item.
For a review-only request, delivering supported findings completes the
requested review report; discovery alone does not authorize repairs.
...
Dispatch independent available review lanes together, using the current delegation API's supported options. If a tool or reviewer is unavailable, follow the compatibility and independence rules above instead of calling unsupported parameters.
...
OUTPUT FORMAT:
<verdict>PASS, FAIL, or INCONCLUSIVE</verdict>
...
Add scenarios only for demonstrated risks or affected boundaries that the existing checks miss. Group them by priority: P0 (must pass), P1 (should pass), P2 (nice to pass). Do not manufacture a scenario count or expand the implementation scope.
...
If the app cannot be started, distinguish an evidenced product/build defect from unavailable environment, credentials, or tooling. Report a defect as FAIL and missing runtime evidence as INCONCLUSIVE; continue independent applicable checks without claiming runtime success.
...
ALL 5 required perspectives have independent PASS evidence → **REVIEW PASSED**
ANY perspective has an evidenced FAIL → **REVIEW FAILED - criteria not met**
