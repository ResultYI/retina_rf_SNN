...
| residual decoder `raw_weight` | initialized to `0` | decoder weight | yes | effective `[-residual_weight_max, residual_weight_max]` via tanh | `effective_weight = residual_weight_max * tanh(raw_weight)` | residual local decoder weights | test-only | `models/decoder/local_decoder.py:206-216`; `tests/test_local_decoder.py:76-85` |
