...
# Preserve the frozen local runtime; do not resolve a different numerical environment.
...
OLD: Final = ROOT / "output/real_data/schottdorf_canonical_v1_overlapping_support_development_22cell_20260830"
...
def main() -> None:
    ...
    for index, expected in enumerate(source.cells):
        ...
        config = MechanisticRetinaConfig(architecture_mode=ArchitectureMode.MECHANISM_IDENTIFIABLE,
                                         cell_specific_gains=True, dt_ms=data.dt_ms)
        ...
        retained_config = {k: v for k, v in serialized_config.items() if k != "causal_contract"}
        assert retained_config == {k: v for k, v in previous["model_config"].items()
                                   if k not in {"ac_basis_tau_ms", "ac_basis_tau_bounds_ms"}}
        assert previous["training_contract"]["learning_rate"] == 0.03
        assert previous["training_contract"]["batch_size"] == 4
        assert previous["training_contract"]["maximum_inner_steps"] == 1000
        assert previous["training_contract"]["patience"] == 200
        assert previous["training_contract"]["min_delta"] == 1e-7
        ...
        metadata = {"schema": "schottdorf_canonical_v1_shared_bc_development",
